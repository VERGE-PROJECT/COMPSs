...
public class NIOAdaptor extends NIOAgent implements CommAdaptor {
    ...
    protected static Map<Integer, Long> taskStartTimes = new HashMap<>();
    ...
    protected void finishJob(NIOJob nj, boolean successful, NIOTaskResult ntr, Exception e) {
        int taskId = nj.getTaskId();
        long time = System.currentTimeMillis();
        NIOProfile p = ntr.getProfile();
        // if task fails for whatever reason, taskStartTimes could not contain taskId key
        if (p != null && this.taskStartTimes.containsKey(taskId)) {
            p.setStartTimeMaster(this.taskStartTimes.get(taskId));
            p.setEndTimeMaster(time);
        }

        nj.taskFinished(successful, ntr, e, p);
    }
    ...
    private void produceFailOnTask(NIOTask task) {
        ...
        if (nj != null) {
            ...
            // Mark task as finished and release waiters
            JobHistory prevJobHistory = nj.getHistory();
            nj.taskFinished(false, null, null, null);
